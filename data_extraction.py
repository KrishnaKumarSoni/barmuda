...
class DataExtractionChain:
    ...
    def extract_responses(self, session_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            extraction_prompt = self._build_extraction_prompt(form_questions, chat_history, existing_responses)
            
            # Call OpenAI for extraction
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are an expert at extracting structured data from conversational text."},
            ...
            processed_data = self._post_process_extraction(extracted_data, form_questions, existing_responses)
    ...
    def _build_extraction_prompt(self, questions: List[Dict], chat_history: List[Dict], existing_responses: Dict) -> str:
...
"""
    ...
    def _post_process_extraction(self, extracted_data: Dict, questions: List[Dict], existing_responses: Dict) -> Dict:
            ...
            try:
                ...
                if idx < len(questions):
                    ...
                    if confidence >= 0.7:
                        ...
                        processed_value = self._process_by_type(response_value, question.get('type', 'text'), question.get('options', []))
    ...
    def _process_by_type(self, value: str, question_type: str, options: List[str]) -> str:
        ...
        if question_type == 'yes_no':
            ...
            if any(word in value_lower for word in ['yes', 'y', 'yeah', 'yep', 'sure', 'ok', 'true', '1']):
                return 'yes'
            elif any(word in value_lower for word in ['no', 'n', 'nope', 'nah', 'false', '0']):
                return 'no'
            else:
                return 'unclear'
            ...
            try:
                ...
                # Try word mapping
                rating_map = {
                    'terrible': '1', 'awful': '1', 'bad': '1', 'poor': '2', 'meh': '2',
                    'okay': '3', 'ok': '3', 'good': '4', 'great': '4', 'excellent': '5',
                    'amazing': '5', 'perfect': '5'
                }
                value_lower = str(value).lower()
