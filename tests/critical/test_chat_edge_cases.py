...
class TestChatAgentEdgeCases:
    ...
    def test_off_topic_redirect_bananas(self, client, mock_db, sample_chat_session, edge_case_messages):
        ...
        with patch('app.db', mock_db), \
            ...
            for i, message in enumerate(edge_case_messages["off_topic"][:3]):
                ...
                assert 'bananas' in data['response'].lower() or 'focus' in data['response'].lower()
    ...
    def test_skip_question_handling(self, client, mock_db, sample_chat_session, edge_case_messages):
        ...
        with patch('app.db', mock_db), \
            ...
            for skip_message in edge_case_messages["skip_requests"]:
                ...
                assert response.status_code == 200
                data = json.loads(response.data)
                assert any(word in data['response'].lower() for word in ['skip', 'cool', 'next'])
    
    def test_multi_answer_parsing(self, client, mock_db, sample_chat_session, edge_case_messages):
        ...
        with patch('app.db', mock_db), \
            ...
            response = client.post('/api/chat/message',
                json={
                    'session_id': 'session_123',
                    'message': multi_answer
                })
            
            assert response.status_code == 200
            data = json.loads(response.data)
            assert 'alex' in data['response'].lower()
            assert 'noted' in data['response'].lower() or 'use' in data['response'].lower()
    
    def test_conflicting_answer_resolution(self, client, mock_db, sample_chat_session, edge_case_messages):
        ...
        with patch('app.db', mock_db), \
            ...
            response1 = client.post('/api/chat/message',
                json={
                    'session_id': 'session_123',
            ...
            response2 = client.post('/api/chat/message',
                json={
                    'session_id': 'session_123',
                    'message': edge_case_messages["conflicting"][1]  # "Actually, no I don't"
                })
            
            assert response2.status_code == 200
            data = json.loads(response2.data)
            assert 'updating' in data['response'].lower() or 'got it' in data['response'].lower()
    
    def test_vague_response_follow_up(self, client, mock_db, sample_chat_session, edge_case_messages):
        ...
        with patch('app.db', mock_db), \
            ...
            for vague_response in edge_case_messages["vague"]:
                ...
                response = client.post('/api/chat/message',
                    json={
                        'session_id': 'session_123',
                        'message': vague_response
                    })
                
                assert response.status_code == 200
                data = json.loads(response.data)
                assert any(word in data['response'].lower() for word in ['specific', 'scale', 'more'])
    ...
    def test_invalid_input_type_handling(self, client, mock_db, sample_chat_session):
        ...
        with patch('app.db', mock_db), \
            ...
            assert response.status_code == 200
            data = json.loads(response.data)
            assert any(word in data['response'].lower() for word in ['several', 'like', '2', '3'])
    ...
    def test_session_timeout_partial_save(self, client, mock_db, sample_chat_session):
        ...
        with patch('app.db', mock_db), \
            ...
            mock_db.collection.return_value.document.return_value.get.return_value.to_dict.return_value = timeout_session
            mock_db.collection.return_value.document.return_value.get.return_value.exists = True
    ...
    def test_message_limit_enforcement(self, client, mock_db, sample_chat_session):
        ...
        with patch('app.db', mock_db), \
            ...
            mock_db.collection.return_value.document.return_value.get.return_value.to_dict.return_value = limit_session
            mock_db.collection.return_value.document.return_value.get.return_value.exists = True
    ...
    def test_multi_language_response_handling(self, client, mock_db, sample_chat_session):
    ...
    def test_agent_tool_execution(self, client, mock_db, sample_chat_session):
        ...
        with patch('app.db', mock_db), \
            ...
            assert response.status_code == 200
            data = json.loads(response.data)
            assert 'next' in data['response'].lower() or 'got it' in data['response'].lower()