...
class TestActiveInactiveControl:
    ...
    def test_active_form_allows_chat_start(self, client, mock_db, sample_active_form):
        ...
        with patch('app.db', mock_db):
            mock_db.collection.return_value.document.return_value.get.return_value.to_dict.return_value = sample_active_form
            mock_db.collection.return_value.document.return_value.get.return_value.exists = True
    ...
    def test_inactive_form_page_shows_unavailable_message(self, client, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            assert response.status_code == 200
            assert b'Survey Not Available' in response.data or b'not available' in response.data.lower()
    
    def test_active_form_page_loads_chat_interface(self, client, mock_db, sample_active_form):
        ...
        with patch('app.db', mock_db):
            ...
            assert b'chat' in response.data.lower() or b'message' in response.data.lower()
    
    def test_status_toggle_changes_form_availability(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            # Toggle to active
            response = authenticated_session.put('/api/forms/test_form_123/status',
                json={'active': True})
    ...
    def test_inactive_form_blocks_chat_messages(self, client, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            mock_db.collection.return_value.document.return_value.get.return_value.to_dict.return_value = mock_session
    ...
    def test_dashboard_shows_all_forms_regardless_of_status(self, authenticated_session, mock_db):
        """Test that dashboard shows both active and inactive forms"""
        forms = [
            {"id": "form1", "title": "Active Form", "active": True, "creator_id": "test_user_123"},
            {"id": "form2", "title": "Inactive Form", "active": False, "creator_id": "test_user_123"}
        ]
        ...
        with patch('app.db', mock_db):
            ...
            mock_query.stream.return_value = [Mock(to_dict=lambda: form, id=form["id"]) for form in forms]
    ...
    def test_form_status_requires_authentication(self, client, mock_db):
        ...
        with patch('app.db', mock_db):
            response = client.put('/api/forms/test_form_123/status',
                json={'active': True})
