...
class TestFormAutoSave:
    ...
    def test_inference_auto_saves_as_inactive(self, authenticated_session, mock_db):
        ...
        with patch('app.db', mock_db), \
            ...
            mock_openai.chat.completions.create.return_value.choices[0].message.content = json.dumps(mock_form_data)
            
            # Mock database save
            mock_db.collection.return_value.add.return_value = (None, "auto_saved_form_123")
            
            response = authenticated_session.post('/api/infer',
                json={'dump': 'Create a customer feedback survey'})
            ...
            save_call = mock_db.collection.return_value.add.call_args[0][0]
            assert save_call['active'] is False  # Must be inactive
            assert save_call['creator_id'] == 'test_user_123'
            assert 'created_at' in save_call
    
    def test_template_selection_auto_saves(self, authenticated_session, mock_db):
        """Test that template selection auto-saves the form"""
        with patch('app.db', mock_db):
            mock_db.collection.return_value.add.return_value = (None, "template_form_123")
            
            response = authenticated_session.post('/api/infer',
                json={'template': 'customer_feedback'})
    ...
    def test_auto_saved_form_appears_in_dashboard(self, authenticated_session, mock_db):
    ...
    def test_edit_form_loads_auto_saved_by_url(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            assert b'test_form_123' in response.data or sample_form["title"].encode() in response.data
    
    def test_save_and_launch_activates_auto_saved_form(self, authenticated_session, mock_db, sample_form):
    ...
    def test_auto_save_prevents_data_loss_on_navigation(self, authenticated_session, mock_db):
        ...
        with patch('app.db', mock_db), \
            ...
            mock_openai.chat.completions.create.return_value.choices[0].message.content = json.dumps({
                ...
                "questions": [{"text": "How was the event?", "type": "text", "enabled": True}]
            })
            
            # Mock auto-save
            mock_db.collection.return_value.add.return_value = (None, "navigated_form_123")
            
            # User creates form via inference
            inference_response = authenticated_session.post('/api/infer',
                json={'dump': 'Create an event feedback form'})
            ...
            mock_db.collection.return_value.document.return_value.get.return_value.to_dict.return_value = saved_form
            mock_db.collection.return_value.document.return_value.get.return_value.exists = True
            
            edit_response = authenticated_session.get(f'/edit-form?id={form_id}')
            assert edit_response.status_code == 200
    
    def test_auto_save_includes_all_form_data(self, authenticated_session, mock_db):
        """Test that auto-save preserves all form elements"""
        complex_form_data = {
            "title": "Complex Survey",
            "description": "A comprehensive survey",
            "questions": [
                {"text": "Rate us 1-5", "type": "rating", "enabled": True, "options": ["1","2","3","4","5"]},
                {"text": "Your feedback", "type": "text", "enabled": True},
                {"text": "Choose color", "type": "multiple_choice", "enabled": False, "options": ["red","blue"]}
            ],
            "demographics": {
                "enabled": True,
                "age": {"enabled": True},
        ...
        with patch('app.db', mock_db), \
            ...
            mock_openai.chat.completions.create.return_value.choices = [Mock()]
            mock_openai.chat.completions.create.return_value.choices[0].message.content = json.dumps(complex_form_data)
            
            mock_db.collection.return_value.add.return_value = (None, "complex_form_123")
            
            response = authenticated_session.post('/api/infer',
                json={'dump': 'Create a complex survey'})
    ...
    def test_auto_save_fails_gracefully(self, authenticated_session, mock_db):
        ...
        with patch('app.db', mock_db), \
            ...
            mock_db.collection.return_value.add.side_effect = Exception("Database error")
    ...
    def test_auto_saved_form_ownership_validation(self, client, mock_db, sample_form):
    ...
    def test_auto_save_preserves_template_metadata(self, authenticated_session, mock_db):
        """Test that template selection metadata is preserved in auto-save"""
        with patch('app.db', mock_db):
            mock_db.collection.return_value.add.return_value = (None, "template_form_123")
            
            response = authenticated_session.post('/api/infer',
                json={'template': 'employee_survey'})
    ...
    def test_multiple_auto_saves_same_user(self, authenticated_session, mock_db):
        ...
        with patch('app.db', mock_db), \
            ...
            for i, form_data in enumerate(forms_data):
                mock_openai.chat.completions.create.return_value.choices = [Mock()]
                mock_openai.chat.completions.create.return_value.choices[0].message.content = json.dumps(form_data)
                
                mock_db.collection.return_value.add.return_value = (None, f"auto_form_{i}")
                
                response = authenticated_session.post('/api/infer',
                    json={'dump': f'Create survey {i+1}'})
    ...
    def test_auto_save_timestamp_accuracy(self, authenticated_session, mock_db):
        ...
        with patch('app.db', mock_db), \
            ...
            mock_db.collection.return_value.add.return_value = (None, "timestamp_form_123")
            
            response = authenticated_session.post('/api/infer',
                json={'dump': 'Test timestamp'})
