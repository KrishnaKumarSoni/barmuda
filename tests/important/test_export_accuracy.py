...
class TestExportAccuracy:
    ...
    def test_csv_export_basic_structure(self, authenticated_session, mock_db, sample_form, sample_responses):
            ...
            mock_responses_data = [
                {
                    ...
                    "metadata": {"completed_at": "2025-01-01T00:00:00Z", "partial": False}
                },
                {
                    "session_id": "session_2", 
                    "responses": {"0": "3", "1": "Could be better"},
                    "metadata": {"completed_at": "2025-01-01T01:00:00Z", "partial": False}
                }
            ]
    ...
    def test_csv_export_data_accuracy(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            assert "More features, please!" in data_row  # Text response with punctuation
    
    def test_csv_export_handles_special_characters(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            csv_content = response.data.decode('utf-8')
            assert 'Text with "quotes"' in csv_content or '"Text with ""quotes""' in csv_content
    ...
    def test_json_export_data_types_preserved(self, authenticated_session, mock_db, sample_form):
    ...
    def test_export_partial_responses_flagged(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            mixed_responses = [
                ...
                },
                {
                    ...
                    "metadata": {"completed_at": "2025-01-01T00:30:00Z", "partial": True}
                }
            ]
            ...
            partial_response = next(r for r in json_data['responses'] if r['session_id'] == 'partial_1')
            assert partial_response['metadata']['partial'] is True
    
    def test_export_demographics_inclusion(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            mock_db.collection.return_value.document.return_value.get.return_value.to_dict.return_value = form_with_demographics
            mock_db.collection.return_value.document.return_value.get.return_value.exists = True
    ...
    def test_export_metadata_accuracy(self, authenticated_session, mock_db, sample_form):
    ...
    def test_export_large_dataset_performance(self, authenticated_session, mock_db, sample_form):
    ...
    def test_export_duplicate_detection(self, authenticated_session, mock_db, sample_form):
    ...
    def test_export_file_naming_convention(self, authenticated_session, mock_db, sample_form):
        ...
        with patch('app.db', mock_db):
            ...
            assert 'test_form_123' in content_disposition or 'Customer Feedback Survey' in content_disposition
