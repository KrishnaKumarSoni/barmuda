...
def test_openai_key():
    ...
    if api_key:
        print_result(
            True, "OpenAI API key found", f"Key length: {len(api_key)} characters"
        )
        return True
    else:
        print_result(
            False, "OpenAI API key not found", "Please set OPENAI_API_KEY in .env file"
        )
        return False
    ...
    try:
        ...
        for case in test_cases:
            ...
            if success:
                ...
                print_result(
                    False,
                    f"Validation test: {case['name']}",
                    error or "Unexpected result",
                )

        print_result(
            passed == len(test_cases),
            f"JSON validation tests",
...
def test_prompt_creation():
    ...
    try:
        ...
        success = (
            ...
            and len(prompt) > 1000  # Should be a substantial prompt
            and test_input in prompt  # Should contain the input
            and "Chain-of-Thought" in prompt  # Should have CoT instructions
            and "FEW-SHOT EXAMPLES" in prompt  # Should have examples
        )

        print_result(
            success, "Prompt creation", f"Prompt length: {len(prompt)} characters"
        )
        return success
        ...
        for case in test_cases:
            ...
            try:
                ...
                if inferred_form:
                    ...
                    has_title = (
                        "title" in inferred_form and len(inferred_form["title"]) > 0
                    )
                    has_questions = (
                        "questions" in inferred_form
                        and len(inferred_form["questions"]) >= case["min_questions"]
                    )

                    # Question type validation
                    question_types = [q["type"] for q in inferred_form["questions"]]
                    has_expected_types = any(
                        qtype in question_types for qtype in case["expected_types"]
                    )
                ...
                print_result(
                    False, f"Inference: {case['name']}", f"Exception: {str(e)}"
                )

        print_result(
            passed == len(test_cases),
            f"Form inference tests",
            f"{passed}/{len(test_cases)} passed",
        )
        return passed == len(test_cases)
...
def test_edge_cases():
    ...
    try:
        ...
        edge_cases = [
            {"name": "Very short input", "input": "survey", "should_succeed": True},
            {
            ...
            try:
                ...
                if success:
                    ...
                    if inferred_form:
                        ...
                        print_result(
                            True, f"Edge case: {case['name']}", "Correctly failed"
                        )
                else:
                    print_result(
                        False,
                ...
                print_result(
                    False, f"Edge case: {case['name']}", f"Exception: {str(e)}"
                )

        print_result(
            passed >= len(edge_cases) * 0.7,
            f"Edge case tests",
            f"{passed}/{len(edge_cases)} passed (70% threshold)",
        )
