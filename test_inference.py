...
def test_health_check():
    ...
    try:
        ...
        print_result(
            ...
            f"OpenAI key present: {data.get('openai', False)}",
        )
        return success
    ...
    for example in test_examples:
        ...
        try:
            ...
            if inferred_form:
                    ...
                    for i, question in enumerate(inferred_form["questions"]):
                        ...
                        for field in required_fields:
                            if field not in question:
                                print_result(
                                    False, f"Question {i+1} missing '{field}' field"
                                )
                                success = False
                ...
                results.append(
                    {"example": example["name"], "success": False, "error": error}
                )

        except Exception as e:
            print_result(False, f"Exception during inference", str(e))
            results.append(
                {"example": example["name"], "success": False, "error": str(e)}
            )
...
def test_edge_cases():
    ...
    edge_cases = [
        {"name": "Empty input", "dump": "", "should_fail": True},
        {"name": "Very short input", "dump": "survey", "should_fail": False},
        {
        ...
        try:
            ...
            if case["name"] == "Empty input":
                ...
                if case["should_fail"]:
                    ...
                    print_result(
                        success,
                        f"Expected failure: {'Got failure' if success else 'Got success'}",
                    )
                else:
...
def generate_test_report(all_results):
    ...
    if inference_results:
        successful_inferences = [
            r for r in inference_results if r.get("success", False)
        ]
        if successful_inferences:
            avg_questions = sum(
                r.get("questions_count", 0) for r in successful_inferences
            ) / len(successful_inferences)
            print(f"Average questions per form: {avg_questions:.1f}")
            print(
                f"Inference success rate: {len(successful_inferences)/len(inference_results)*100:.1f}%"
            )
