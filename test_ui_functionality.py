...
class BarmudaUITester:
    ...
    def test_homepage_ui(self):
        ...
        try:
            ...
            cta_button = self.driver.find_elements(
    ...
    def test_login_flow(self):
        ...
        try:
            ...
            if sign_in_elements:
                ...
                auth_flow = (
                    "google" in current_url.lower() or "firebase" in current_url.lower()
                )

                self.log_test(
                    "Login Flow Trigger", auth_flow, f"Redirected to: {current_url}"
                )
                return auth_flow
            else:
    ...
    def test_form_creation_ui_manual(self):
        ...
        try:
            ...
            if "Authentication" in page_title or "login" in page_source.lower():
                ...
                has_form_elements = (
                    len(textarea_elements) > 0 or len(input_elements) > 0
                )

                self.log_test(
                    "Form Creation UI Elements",
                    has_form_elements,
    ...
    def test_chat_ui_structure(self):
        ...
        try:
            ...
            if "not found" in page_title.lower() or "not found" in page_source.lower():
                self.log_test(
                    "Chat UI Structure (404)",
                    True,
                    "Proper 404 handling for non-existent form",
                )
                return True
            else:
                ...
                chat_elements.extend(
                    self.driver.find_elements(By.CLASS_NAME, "message")
                )
                chat_elements.extend(self.driver.find_elements(By.ID, "chat"))
                chat_elements.extend(
                    self.driver.find_elements(By.XPATH, "//input[@type='text']")
                )
    ...
    def test_mobile_responsiveness(self):
        ...
        try:
            ...
            for width, height in screen_sizes:
                ...
                print(
                    f"   {width}x{height}: {'✅' if no_horizontal_scroll else '❌'} (Page: {page_width}px, Viewport: {viewport_width}px)"
                )
