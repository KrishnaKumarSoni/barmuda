...
def test_protected_route_without_auth():
    ...
    try:
        ...
        assert response.status_code in [401, 403, 302], f"Expected auth error, got {response.status_code}"
...
def test_auth_verify_with_invalid_token():
    ...
    try:
        response = requests.post(f"{BASE_URL}/auth/verify", json={"idToken": "invalid-token"})
        assert response.status_code == 401, f"Expected 401, got {response.status_code}"
...
if __name__ == "__main__":
    ...
    if sys.stdin.isatty():
        print("⚠️  Make sure to start the Flask app with 'python app.py' before running tests!")
        print("Press Enter to continue or Ctrl+C to cancel...")
        input()
    else:
        print("🤖 Running in automated mode - assuming Flask app is running on localhost:5000")
